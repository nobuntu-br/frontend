...
import { DatabasePermission } from './databaseCredential/databasePermission.model';
...
export class TenantService extends BaseResourceService<ITenant> {
  ...
  async getTenantsAndSaveInLocalStorage(userUID: string): Promise<DatabasePermission[] | null>{
    ...
    try {
      let tenants: DatabasePermission[] = await firstValueFrom(this.http.get<DatabasePermission[]>(this.url + "/uid/" + userUID));
  ...
  getTenantsUserIsAdmin(userUID: string): Observable<Tenant[]>{
    return this.http.get(this.url + "/isAdmin/").pipe(
      map(this.jsonDataToResources.bind(this)),
      catchError(this.handleError)
    )
  }
