...
@Component({
  selector: 'app-list-order-details-status',
  templateUrl: './list-order-details-status.component.html',
  styleUrls: ['./list-order-details-status.component.scss']
})
export class ListOrderDetailsStatusComponent implements AfterViewInit,  OnDestroy  {
  ...
  userConfig; 
  searchableFields = [] 
  //apiURL: string = environment.OrderDetailsStatusAPIPath;//Campo que é alterável, o nome do campo "variations"+"APIPATH". 
  JSONURL: string = environment.orderDetailsStatusJSONPath;// Campo que é alterável, o nome do campo "variations"+"JSONPATH". 
  /** 
