...
@Component({
  selector: 'app-list-orders-status',
  templateUrl: './list-orders-status.component.html',
  styleUrls: ['./list-orders-status.component.scss']
})
export class ListOrdersStatusComponent implements AfterViewInit,  OnDestroy  {
  ...
  userConfig; 
  searchableFields = [] 
  //apiURL: string = environment.OrdersStatusAPIPath;//Campo que é alterável, o nome do campo "variations"+"APIPATH". 
  JSONURL: string = environment.ordersStatusJSONPath;// Campo que é alterável, o nome do campo "variations"+"JSONPATH". 
  /** 
